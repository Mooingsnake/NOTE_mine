...
时间复杂度衡量的是算法执行所需要的**计算步骤（操作次数）**随数据量 $n$ 的增长趋势。

#### 常见时间复杂度一览

| 复杂度 | 名称 | 典型场景 / 算法 | 举例 |
| :--- | :--- | :--- | :--- |
| **$O(1)$** | 常数阶 | 数组随机访问、哈希表查找 | 无论数组多大，直接通过下标 `arr[i]` 获取元素 |
| **$O(\log n)$** | 对数阶 | 二分查找、平衡树/红黑树 | 每一步将查找范围减半 |
| **$O(n)$** | 线性阶 | 单层循环遍历 | 遍历一次长度为 $n$ 的数组或链表 |
| **$O(n \log n)$** | 线性对数阶 | 快速排序、归并排序、堆排序 | 结合分治与线性遍历的高效排序算法 |
| **$O(n^2)$** | 平方阶 | 双重嵌套循环 | 冒泡排序、选择排序、插入排序 |
| **$O(2^n)$** | 指数阶 | 递归求解斐波那契数列 | 暴力穷举子集 |

---

### 2. 深入理解：线性对数阶 $O(n \log n)$ 是怎么算出来的？

线性对数阶 $O(n \log n)$ 实际上是 **线性阶 $O(n)$** 和 **对数阶 $O(\log n)$** 的结合，常见于**“分治法”（Divide and Conquer）**。

以最典型的**归并排序（Merge Sort）**为例，它的推导分为两个维度：

#### ① 垂直维度：分解与递归的层数 —— $O(\log n)$
* 假设有 $n$ 个元素的数组，每次从中间切一刀，分成左右两半。
* 1 个长数组 $\rightarrow$ 2 个半长数组 $\rightarrow$ 4 个四分之一数组 $\dots \rightarrow n$ 个长度为 1 的单元素数组。
* 这个切分过程可以构建成一棵**二叉树**。对于 $n$ 个节点，树的高度（即需要拆分的次数）就是 $\log_2 n$。
* 在大O表示法中省略底数，即拆分层数为 **$O(\log n)$**。

#### ② 水平维度：每层合并时的比较与复制 —— $O(n)$
* 在每一层递归树中，我们需要把两半有序的子数组合并成一个大的有序数组。
* 无论是第一层合并 $n$ 个元素，还是下面各层合并若干小数组，**每一层所有小数组合并的元素总量加起来永远是 $n$**。
* 因此，处理完树的其中“一层”，所花费的操作次数就是 **$O(n)$**。

#### ③ 整体计算：总步骤 = 层数 $\times$ 每层工作量
$$\text{总时间复杂度} = \text{递归树高度 } O(\log n) \times \text{每层合并工作量 } O(n) = O(n \log n)$$

> **直观理解**：相当于对大小为 $n$ 的数据做了一共 $\log n$ 轮处理，每一轮都需要把这 $n$ 个数据扫描/操作一遍，因此总消耗就是 $n \times \log n$。

---

## 3. 空间复杂度

空间复杂度衡量的是算法在运行过程中**额外占用的内存空间**随数据量 $n$ 的增长趋势。

* **只算额外空间**：输入的原始数据本身占用的内存不计入算法的空间复杂度，只计算算法为了运行而额外申请的数组、结构体或递归调用栈空间。
* **常见空间复杂度**：
  * **$O(1)$**：只用了几个变量做临时存储（如冒泡排序、原地交换）。
  * **$O(n)$**：开辟了与输入同等规模的辅助数组，或者深度为 $n$ 的递归调用栈（如归并排序的辅助数组）。

---

## 核心总结

在实际开发和算法设计中，时间和空间常常不可兼得。许多优秀的算法都是通过**“空间换时间”**（例如哈希表、动态规划缓存表）来提高运行速度的。
...
比如序列5 8 5 2 9， 我们知道第一遍选择第1个元素5会和2交换，那么原序列中2个5的相对前后顺序就被破坏了，所以选择排序不是一个稳定的排序算法
### 插入排序 O(n²)
...
已知先序遍历为abcd，求有多少个二叉树？

https://blog.csdn.net/KK_2018/article/details/107597647
### 二叉搜索树
...
### 红黑树、
比较好的讲解：https://www.bilibili.com/video/BV135411h7wJ
